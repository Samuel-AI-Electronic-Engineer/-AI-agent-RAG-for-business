    ...
    return (
        <main className="page page-dashboard profile-subpage">
            ...
            <section className="dashboard-card profile-card subpage-card">
                ...
                {orders.length === 0 ? <div className="empty-state"><p>Aquí aparecerán tus pedidos cuando completes una compra.</p><Link to="/libreria" className="btn-ghost compact-button">Explorar librería</Link></div> : <div className="order-list">{orders.map((order) => <article className="order-row order-row-large" key={order.id}><span><strong>{order.id}</strong><small>{new Date(order.date).toLocaleDateString('es-CO')}</small></span><span>{order.items.length} {order.items.length === 1 ? 'edición' : 'ediciones'}</span><strong>{order.status}</strong></article>)}</div>}
            </section>
        </main>
    );
}
